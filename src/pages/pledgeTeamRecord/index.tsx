...
import {APIStakingTeamOrder} from "@/api";
  ...
  return (
    <div className={styles.record_item_wrap}>
      ...
      <div className={styles.info_wrap}>
        ...
        <div className={styles.info_value_common_text}>{t('pledge_modal_will_receive_daily_amount', {amount: numberFormatToEnglish((props.stakingRecordItem.dailyRatio / 10000) * (props.stakingRecordItem.amount ?? 0) * (props.stakingRecordItem.hours / 24) / 100, 4), currency: MAIN_CURRENCY_COIN})}</div>
      </div>
      <div className={styles.info_wrap}>
      ...
      {
        isDone && (
          <div className={styles.left_tag_down}><span>Done</span></div>
        )
      }
...
const StakingTeamRecord = () => {
  ...
  const fetchData = () => {
    ...
    APIStakingTeamOrder(params).then(resp => {
      if (resp.code === 0) {
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header_wrap}>
        <span>{t('pledge_team_details')}</span>
      </div>
...
export default StakingTeamRecord
